...
public class JDBCRun {
	...
	public static void main(String[] args) {
		// SQL Developer가 잘 동작하는지 확인하기
		/*
		 * ***JDBC 코딩 절차***
		 * 1. 드라이버 등록 (ojdbc6.jar)
		 * 2. DBMS 연결 생성 (Driver Manager)
		 * 3. Statement 객체 생성 (쿼리문 실행 준비)
		 *  - new Statement(); 가 아니라 연결을 통해 객체 생성함. (Statement는 인터페이스)
		 * 4. SQL 전송 (쿼리문 실행)
		...
		try {
			String url 		= "jdbc:oracle:thin:@localhost:1521:xe"; // : 붙이기
			String user 	= "KH";
			String password = "KH";
			String query = "SELECT EMP_NAME, SALARY FROM EMPLOYEE"; // ; 빼기
			// 1. 드라이버 등록
			...
			while(rset.next()) {
				...
				// 컬럼의 데이터 타입이 문자면 getString, 숫자면 getInt
			}
			// 6. 자원해제
			rset.close();
			stmt.close();
